...
exports.generate = function(alg, enc, payload, ...rest) {
...
function decryptJwe(parsed, aMatch, eMatch, key, cb) {
  ...
  // Optional inflate
  if (parsed.header.zip === 'DEF') {
    try {
      plain = zlib.inflateRawSync(Buffer.from(plain))
    } catch (error) {
      parsed.error = { message: `Could not inflate plain payload. ${error.message}` };
      return responder(null, parsed, cb);
    }
  }
  try {
